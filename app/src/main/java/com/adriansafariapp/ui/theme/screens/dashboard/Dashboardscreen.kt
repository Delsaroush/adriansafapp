package com.adriansafariapp.ui.theme.screens.dashboard
...
import com.adriansafariapp.data.DashboardViewModel
import com.adriansafariapp.model.DashboardStat
import com.adriansafariapp.model.QuickAction

@Composable
fun Dashboard_Screen(viewModel: DashboardViewModel= viewModel()) {
    val stats = viewModel.stats.collectAsState().value
    val action = viewModel.quickAction.collectAsState().value
...
fun ActionItem(action: QuickAction) {
    Card (
        ...
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = Color(0xFFF5F5))
    ){
        Row (
